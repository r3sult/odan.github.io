---
title: ASP.NET Core 2.x Resources
date: 2018-07-12
layout: post
comments: true
published: true
description: 
keywords: 
---

## Websites

* https://docs.microsoft.com/en-us/aspnet/core/?view=aspnetcore-2.1
* [.NET Core 2.1 SDK](https://www.microsoft.com/net/download/dotnet-core/sdk-2.1.300)
* [.NET Core 2.1 Runtime](https://www.microsoft.com/net/download/dotnet-core/runtime-2.1.0)
* https://en.wikipedia.org/wiki/ASP.NET_Core

## Books

* https://www.amazon.de/dp/B076DMK61S
* https://www.manning.com/books/dotnet-core-in-action

## Workshops

* [ASP.NET CORE for Beginners - an 8 Hour Workshop](https://www.youtube.com/watch?v=--lYHxrsLsc)

## IDE

* [Visual Studio Code](https://code.visualstudio.com/)
* [JetBrains Rider](https://www.jetbrains.com/rider/) is a new cross-platform .NET IDE based on IntelliJ Platform and ReSharper

## Packages

### Templates

* [ASP.NET Razor](https://en.wikipedia.org/wiki/ASP.NET_Razor)
* [C# Razor Syntax Quick Reference](https://haacked.com/archive/2011/01/06/razor-syntax-quick-reference.aspx/)

### Database

* https://www.nuget.org/packages/EntityFramework/
* https://www.nuget.org/packages/MySql.Data/
* [SqlKata - The powerful c# sql query builder](https://sqlkata.com/docs)

### Reporting

* https://www.stimulsoft.com/en Reporting
* [iTextSharp](https://www.nuget.org/packages/iTextSharp/) PDF library 

### JSON

* [Newtonsoft.Json](https://www.nuget.org/packages/Newtonsoft.Json/)

### Testing

* https://www.nuget.org/packages/NUnit/

### Frontent

* https://www.nuget.org/packages/bootstrap/
* https://www.nuget.org/packages/jQuery/
* [Faster Frontends in ASP.NET Core](https://www.telerik.com/blogs/faster-frontends-in-asp-net-core)

## Hosting

* https://azure.microsoft.com/en-us/
* https://docs.microsoft.com/en-us/aspnet/core/host-and-deploy/iis/?view=aspnetcore-2.1&tabs=aspnetcore2x
* https://docs.microsoft.com/en-us/aspnet/core/host-and-deploy/?view=aspnetcore-2.1&tabs=aspnetcore2x
* http://www.hanselman.com/blog/PublishingAnASPNETCoreWebsiteToACheapLinuxVMHost.aspx
* https://www.strato.de/server/windows-vserver/
* https://hosting.1und1.de/windows-hosting
* https://www.webwiz.net/hosting/asp.net-core-2.0-hosting.htm

## News

* https://www.dotnetweekly.com/
* https://www.c-sharpcorner.com/technologies/dotnetcore-news
* https://www.infoq.com/Net-Core/news
* https://www.reddit.com/r/dotnet/
* https://twitter.com/dotnet
* https://twitter.com/hashtag/netcore
* https://blogs.msdn.microsoft.com/dotnet/